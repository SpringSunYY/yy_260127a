...
import static com.lz.module.system.service.user.AdminUserServiceImpl.USER_INIT_PASSWORD_KEY;
...
public class AdminUserServiceImplTest extends BaseDbUnitTest {
    ...
    public void before() {
        // mock 初始化密码
        when(configApi.getConfigValueByKey(USER_INIT_PASSWORD_KEY)).thenReturn("projectyuanma");
    }
    ...
    public void testCreatUser_success() {
        ...
        when(passwordEncoder.encode(eq(reqVO.getPassword()))).thenReturn("projectyuanma");
        ...
        assertEquals("projectyuanma", user.getPassword());
    ...
    public void testImportUserList_02() {
        ...
        when(deptService.getDept(eq(dept.getId()))).thenReturn(dept);
        // mock passwordEncoder 的方法
        when(passwordEncoder.encode(eq("projectyuanma"))).thenReturn("java");
