...
public class CodegenEngine {
    ...
    /**
     * 是否为 project-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 project-module-xxx-api 模块
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/project-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/project-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "project-module-${table.moduleName}/" + // 顶级模块
                "project-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "project-module-${table.moduleName}/" + // 顶级模块
                "project-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "project-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "project-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
